<template>
  <div class="bg-white rounded-2xl shadow p-4">
    <h3 class="font-semibold mb-2">{{ model?.id ? 'Edit Employee' : 'New Employee' }}</h3>
    <form class="grid gap-3" @submit.prevent="onSave">
      ...
      <select v-model="local.role" class="border p-2 rounded">
...
const props = defineProps<{ model: Employee | null }>()
const emit = defineEmits<{ (e: 'save', item: Employee): void; (e: 'cancel'): void }>()

const local = reactive<Employee>({ name: '', role: 'OTHER', email: '' })
